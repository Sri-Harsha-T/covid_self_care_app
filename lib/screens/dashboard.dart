...
import 'package:covid_self_care_app/components/reusable_widgets.dart';
import 'package:covid_self_care_app/constants.dart';
import 'package:covid_self_care_app/countries_list.dart';
...
class _DashboardState extends State<Dashboard> {
  ...
  Future getCountryData(country) async {
    var response =
        await http.get('https://disease.sh/v3/covid-19/countries/$country' as Uri);
    return (jsonDecode(response.body));
  }

  Future getWorldData() async {
    try {
      var response = await http.get('https://disease.sh/v3/covid-19/all' as Uri);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: AppBarContent(title: 'Covid-19 Tracker', icon: Icon(Icons.whatshot),),
      ),
      body: SafeArea(
        child: ListView(
