...
class BarChartDaily extends StatefulWidget {
  ...
  BarChartDaily({required Key key, required this.name, required this.totalValue, required this.dailyData, required this.chartColor}) : super(key: key);
...
class _BarChartDailyState extends State<BarChartDaily> {
  ...
  late List<CustomRadioModel> radioModels = [];
  late List<SingleDayData> singleDayData = [];
  late int maxDailyValue;
                ...
                children: <Widget>[
                  ...
                  isSpotSelected
                      ? DateValue(
                          ...
                          chartColor: chartColor, key: ObjectKey(""),
                        )
                      : Container(
                          height: 28,
                        ),
  ...
  BarChartData buildBarChart(List<SingleDayData> singleDayData, BuildContext context) {
    ...
    return BarChartData(
      ...
      barTouchData: BarTouchData(
        ...
        touchCallback: (FlTouchEvent flTouchEvent, barTouchResponse) {
          //todo: show detailed data
          print("TouchInputOffset: " + barTouchResponse!.spot!.offset.toString());
          if (barTouchResponse.spot != null && barTouchResponse.spot is! FlPanEndEvent && barTouchResponse.spot is! FlLongPressEnd) {
      ...
      barChartGroups.add(
        BarChartGroupData(x: i, barRods: [
          BarChartRodData(
            ...
            colors: getColor(i, touchedSpotXValue),
  ...
  List<SingleDayData> buildChartDataList(List<DailyData> dailyData) {
    List<SingleDayData> singleDayDataList = [];
  ...
  String getBottomTitle({required double index, required int numDays, required List<SingleDayData> data}) {
    if (numDays == 14) {
      if ([
        0,
        3,
  ...
  String getLeftTitle({required double index, required List<SingleDayData> data}) {
